...
class SystemSettings extends DBConnection{
	...
	function update_settings_info(){
		...
		if(isset($_FILES['bannerss']) && count($_FILES['bannerss']['tmp_name']) > 0){
			$err='';
			$banners_path = "uploads/banners/";
			foreach($_FILES['bannerss']['tmp_name'] as $k => $v){
				if(!empty($_FILES['bannerss']['tmp_name'][$k])){
					$accept = array('image/jpeg','image/png');
					if(!in_array($_FILES['bannerss']['type'][$k],$accept)){
						$err = "Image file type is invalid";
						break;
					}
					if($_FILES['bannerss']['type'][$k] == 'image/jpeg')
						$uploadfile = imagecreatefromjpeg($_FILES['bannerss']['tmp_name'][$k]);
					elseif($_FILES['bannerss']['type'][$k] == 'image/png')
						$uploadfile = imagecreatefrompng($_FILES['bannerss']['tmp_name'][$k]);
					if(!$uploadfile){
						$err = "Image is invalid";
						break;
					}
					list($width, $height) =getimagesize($_FILES['bannerss']['tmp_name'][$k]);
					...
					$temp = imagescale($uploadfile,$width,$height);
					$spath = base_app.$banners_path.'/'.$_FILES['bannerss']['name'][$k];
					$i = 1;
					while(true){
						if(is_file($spath)){
							$spath = base_app.$banners_path.'/'.($i++).'_'.$_FILES['banners']['name'][$k];
						}else{
							break;
						}
					}
